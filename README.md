...
```ts
class UserService {
    protected logger = new SomeLogger();  // BAD: logger is a dependency and should be provided to UserService
}

class AccountService {
    constructor(protected logger: Logger) {}   // GOOD: now application can choose which exact logger to provide
}
```
...
On the other hand `AccountService` accepts logger as its constructor argument, thus stating that the consumer of this class must make sure to provide a logging implementation appropriate for that consumer.
